...
# Input
filetypes = (".mp4", ".MP4", ".mkv", ".MKV", ".avi", ".mov", ".wmv")
...
class MainWindow():
    ...
    def get_files(self, folder, filetypes):
        self.list_paths = []
        self.list_filenames = []
        ...
        for file in self.list_dir:
            if file.endswith(filetypes):
                ...
                self.filetype.append('.'+file.split('.')[-1])
...
#        self.list_embarrassing = [0] * len(list_files)
    ...
    def update_page(self, index):
        ...
        self.entry_title.insert(0, self.list_filenames[index][:-(len(self.list_filenames[index].split('.')[-1])+1)])
#        self.var_embarr.set(self.list_embarrassing[index])
    ...
    def load_frames(self, index):
        ...
        if len(self.list_files) > 0:
            for i in range(9):
                name_frame = self.list_files[index][:-(len(self.list_files[index].split('.')[-1])+1)] + '_frame0' + str(i+1) + '.jpg'
...
#        self.list_embarrassing[self.current_index] = self.var_embarr.get()
    ...
    def update_directory(self):
        ...
        self.list_files = self.get_files(self.folder_path.get(), self.filetypes)
...
#                check_output(['ffmpeg-normalize', '-v', self.list_files[i], '-c:a', 'aac', '-b:a', '192k', '-ar', '48000', '-o', self.list_files[i], '-f'])
                ...
                # rename fanart
                frame_fanart = self.list_files[i][:-len(self.filetype[i])] + '_frame0' + str(self.list_fanart[i] + 1) + '.jpg'
                copy(frame_fanart, self.list_files[i][:-len(self.filetype[i])] + '-fanart.jpg')
                # combine poster1 and poster2 to poster
                poster = Image.new('RGB', (954,1080), (0, 0, 0))
                poster1 = Image.open(self.list_files[i][:-len(self.filetype[i])] + '_frame0' + str(self.list_poster1[i] + 1) + '.jpg')
                poster2 = Image.open(self.list_files[i][:-len(self.filetype[i])] + '_frame0' + str(self.list_poster2[i] + 1) + '.jpg')
                ...
                poster.save(self.list_files[i][:-len(self.filetype[i])] + '-poster.jpg')
                # create nfo file
                self.create_nfo(self.list_files[i], stat.st_mtime, self.list_titles[i])
                # move kodi files to subfolder
                basename = self.list_filenames[i][:-len(self.filetype[i])]
                subfolder = os.path.normpath(os.path.join(self.folder,basename))
                try:
                    os.stat(subfolder)
                except:
                    os.mkdir(subfolder)
                move(os.path.normpath(os.path.join(self.folder,basename + self.filetype[i])),os.path.normpath(os.path.join(self.folder,basename,basename + self.filetype[i])))
                ...
                for frame in range(9):
                    try:
                        os.remove(self.list_files[i][:-len(self.filetype[i])] + '_frame0' + str(frame + 1) + '.jpg')
                    except:
                        pass                
    ...
    def create_nfo(self, filepath, timestamp, title):
        path_nfo = filepath[:-(len(filepath.split('.')[-1])+1)] + '.nfo'
        # write nfo file
        nfo = codecs.open(path_nfo, "w", "utf-8")
        nfo.write('<?xml version="1.0" encoding="UTF-8" standalone="yes" ?>' + '\n')
        nfo.write('<movie>' + '\n')
        nfo.write(' <title>' + title + '</title>' + '\n')
        nfo.write(' <sorttitle>' + title + '</sorttitle>' + '\n')
        nfo.write(' <id>-1</id>' + '\n')
        nfo.write(' <genre>XXX</genre>' + '\n')
        nfo.write(' <set>XXX</set>' + '\n')
        nfo.write('</movie>' + '\n')
        nfo.close()
